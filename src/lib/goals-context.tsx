...
// goal"). En mémoire uniquement (pas de backend) : réinitialisé au
// rechargement de la page, suffisant pour une démo de reproduction visuelle.
function GoalsProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = React.useMemo(() => ({ goals, totalSaved, addGoal }), [goals, totalSaved, addGoal])
